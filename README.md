...
- 🌐 **Universal CAD Generation Engine (Any 3D Model Class)**: Built-in parametric synthesis recipes for all major mechanical, consumer, and industrial archetypes: Rotational/Turned shafts & pulleys, Thin-walled enclosures with screw bosses, Spur gears & impellers with polar radial patterns, Swept fluid conduit/elbows, Multi-plane lofted transition ducts, Gusseted L-brackets & U-channels, and Ergonomic consumer goods (phone stands, drinkware mugs, knurled knobs).
- 🧠 **121-Example Universal RAG Vector Store**: Persistent ChromaDB vector store powered by local `sentence-transformers/all-MiniLM-L6-v2` embeddings, providing semantic few-shot retrieval across 121 verified production-grade build123d models spanning every engineering domain.
- 🎯 **Dynamic Archetype Intent Classifier**: Automatically analyzes user prompts with keyword and geometric pattern heuristics to inject specialized architectural constraints and syntax directives directly into LLM synthesis.
...
## 🏛️ Universal CAD Generation Archetypes

The AI CAD Workbench supports generating **all classes of 3D solid geometry** via specialized parametric recipes:

| Archetype | Common Parts | Core build123d Recipe |
|---|---|---|
| **1. Rotational & Axisymmetric** | Stepped drive shafts, V-belt pulleys, bushings, nozzles, bottles, vases | Concentric primitive stacking along Z (`Cylinder`, `Cone`) or 2D sketch `revolve(axis=Axis.Z)` on `Plane.XZ` |
| **2. Enclosures & Housings** | Electronics boxes, sensor enclosures, waterproof shells, cases | `RectangleRounded` outer solid, subtractive hollow cavity leaving wall thickness `T`, and corner mounting bosses |
| **3. Gears & Radial Patterns** | Spur gears, pinions, timing pulleys, centrifugal impellers, fan blades | Central hub cylinder + pitch disc with teeth/vanes arrayed via `PolarLocations` or trigonometric loops |
| **4. Swept Piping & Conduit** | 90° pipe elbows, curved manifolds, exhaust tubing, wire conduits | Cross-section sketch on `Plane.XY` swept along 3D path wire (`CenterArc`), bolt flanges, and inner subtractive bore |
| **5. Lofted Transitions** | Square-to-round HVAC ducts, aerodynamic nozzles, funnels, adapters | `loft(sections=[sk_base, sk_top])` across parallel offset planes (`Plane.XY.offset(H)`) with subtractive core |
| **6. Gusseted Brackets & Channels** | Heavy-duty 90° L-brackets, motor mounts, U-channels, C-channels | Intersecting plates fused inside `BuildPart()` + triangular stiffening ribs (`Polygon` + `extrude`) + slotted holes |
| **7. Consumer & Ergonomic** | Desktop phone/tablet stands, coffee mugs with handles, knurled knobs | Angled cradle geometry, hollow cup cylinders with swept C-handles, perimeter knurling ribs with D-shaft bores |
| **8. Multi-Body Assemblies** | Hex bolt + nut, clevis + pin joints, split bearing pillow blocks | `design_mode: "assembly"`, individual component definitions, mechanical clearances, and multi-solid validation |
...
│   ├── test_ast_security.py  ← AST Sandbox Security Test Suite (9 tests)
...
│   ├── test_universal_archetypes.py ← Universal Archetype Detection, RAG & Execution Tests
│   └── rag_corpus/
...
│       └── examples_universal.py ← 20 Universal Archetype Models (shafts, gears, enclosures, etc.)
└── frontend/
...
| `POST` | `/api/generate` | **Primary:** NL prompt → RAG (121 examples) → LLM → execute → 3D solid *(Rate limited)* |
...
```bash
# 1. Run the full 75-test suite (recommended)
cd backend
.\venv\Scripts\python.exe -m pytest -v

# 2. Run via unified runner (alternative)
.\venv\Scripts\python run_tests.py

# 3. Run only universal archetype tests
.\venv\Scripts\python.exe -m pytest test_universal_archetypes.py -v

# 4. Run the 20-prompt benchmark evaluation
...
| **Member 2** | ChromaDB Vector Store & Local Embeddings (SentenceTransformers), 121-Example RAG Corpus Engineering, Geometry Topology Validation |
