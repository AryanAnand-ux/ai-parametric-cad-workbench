...
async def run_week2_async_tests():
    ...
    sample_cad_code = """import trimesh
...
print(f"Executed async mesh: {l}x{w}x{h}")
"""
...
# Intentional Syntax / Logic Exception
...
"""
    ...
    tasks = [
        CADRunner.execute_script_async(
            ...
            python_code=sample_cad_code,
            parameters={"bracket_length": 40 + i * 10, "width": 20, "height": 10}
        )
        for i in range(3)
    ]
    ...
    for idx, r in enumerate(results):
        assert r["status"] == "success", f"Parallel execution {idx} failed"
