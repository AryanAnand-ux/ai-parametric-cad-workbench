"""
Script to generate and validate backend/rag_corpus/examples_complex.py with 30 high-difficulty CAD examples.
"""
...
# 1. CPU Heatsink
add_ex(
    "cpu_heatsink_fin_array",
    "High-performance linear extruded aluminum CPU heatsink with rectangular fin array, thick heat spreader base plate, and 4 corner mounting screw holes",
    ["heatsink", "heat sink", "cpu", "thermal", "cooling", "fins", "extruded", "heat spreader", "electronics cooling"],
    '''\
PARAMS = {
    "base_length": 80.0,
    "base_width": 60.0,
...
# 2. Cylindrical Motor Stator Housing
add_ex(
    "cylindrical_motor_stator_housing",
    "Brushless electric motor cylindrical stator housing with central bore, front mounting flange with 4-bolt pattern, and longitudinal external cooling ribs",
    ["motor housing", "stator", "brushless", "electric motor", "casing", "cooling ribs", "flange", "powertrain"],
    '''\
PARAMS = {
    "outer_radius": 30.0,
    "inner_radius": 24.0,
...
# 3. Precision Spur Gear Blank
add_ex(
    "spur_gear_blank",
    "Machined industrial spur gear blank with central hub, shaft bore, standard keyway, recessed web, and outer rim with circular lightening holes",
    ["spur gear", "gear blank", "hub", "keyway", "lightening holes", "transmission", "powertrain", "machined"],
    '''\
PARAMS = {
...
add_ex(
    "v_groove_pulley",
    "Single-groove industrial V-belt drive pulley with central hub, shaft bore, keyway slot, web plate, and 38-degree trapezoidal V-groove rim",
    ["pulley", "v-belt", "v-groove", "belt drive", "hub", "keyway", "sheave", "mechanical drive"],
    '''\
PARAMS = {
    "outer_dia": 100.0,
    "groove_depth": 14.0,
...
# 5. Weld Neck Pipe Flange
add_ex(
    "weld_neck_pipe_flange",
    "Class 150 weld neck pipe flange with raised face, through bore, tapered welding neck hub, and 8-bolt circle pattern",
    ["pipe flange", "flange", "weld neck", "piping", "valve", "raised face", "pressure vessel", "bolt circle"],
    '''\
PARAMS = {
    "flange_od": 150.0,
...
# 6. Turbine Compressor Rotor Disc
add_ex(
    "turbine_compressor_disc_blank",
    "Gas turbine or turbocharger centrifugal compressor rotor disc blank with central shaft bore, curved hub, rim, and circular balancing holes",
    ["turbine disc", "compressor disc", "rotor", "turbocharger", "aerospace", "impeller blank", "blisk", "turbomachinery"],
    '''\
PARAMS = {
...
# 7. Electronics Enclosure Base with Standoffs
add_ex(
    "electronics_enclosure_standoffs",
    "Rectangular electronics project box bottom enclosure with rounded corners, hollow interior, 4 corner PCB screw standoff bosses, and side cable gland cutout",
    ["enclosure", "electronics box", "chassis", "pcb standoff", "boss", "cable gland", "housing", "sheet metal box"],
    '''\
PARAMS = {
    "box_length": 110.0,
    "box_width": 75.0,
...
# 8. Heat Exchanger Tube Sheet
add_ex(
    "heat_exchanger_tube_sheet",
    "Shell-and-tube heat exchanger circular tube sheet plate with peripheral flange bolt holes and perforated hexagonal/triangular pitch tube hole array",
    ["tube sheet", "heat exchanger", "perforated", "boiler", "condenser", "baffle", "thermal engineering", "tube bundle"],
    '''\
PARAMS = {
...
# 9. Flexible Spider / Jaw Coupling Hub
add_ex(
    "jaw_coupling_hub",
    "Three-jaw flexible spider shaft coupling hub with central bore, keyway, clamping slit, and interlocking curved drive jaws",
    ["jaw coupling", "coupling", "spider coupling", "shaft coupling", "drive jaw", "flexible coupling", "powertrain"],
    '''\
PARAMS = {
    "outer_radius": 35.0,
    "hub_height": 25.0,
...
# 10. Stepped Transmission Shaft
add_ex(
    "stepped_drive_shaft",
    "Three-step mechanical transmission drive shaft with precision bearing journals, central gear seating shoulder, keyway, and retaining circlip groove",
    ["stepped shaft", "drive shaft", "transmission shaft", "shaft", "bearing journal", "keyway", "circlip groove", "axle"],
    '''\
PARAMS = {
    "d_small": 20.0,
    "len_small": 35.0,
...
# 11. Hydraulic Manifold Block
add_ex(
    "hydraulic_manifold_block",
    "High-pressure hydraulic valve subplate manifold block with standard P, T, A, B port counterbores, internal galleries, and mounting holes",
    ["hydraulic manifold", "valve block", "fluid power", "subplate", "hydraulic ports", "o-ring counterbore", "manifold"],
    '''\
PARAMS = {
    "block_length": 90.0,
    "block_width": 60.0,
...
# 12. Internal Combustion Engine Rocker Arm
add_ex(
    "engine_rocker_arm",
    "Engine valvetrain rocker arm with central fulcrum pivot boss bore, pushrod socket end, and roller valve stem actuating pad",
    ["rocker arm", "valvetrain", "engine", "fulcrum", "pushrod", "valve", "clevis", "automotive mechanical"],
    '''\
PARAMS = {
...
# 13. Supersonic Rocket Conical Nozzle
add_ex(
    "rocket_conical_nozzle",
    "Convergent-divergent supersonic de Laval conical rocket nozzle with combustion chamber injector flange, throat, and conical expansion bell",
    ["rocket nozzle", "de laval", "convergent divergent", "supersonic nozzle", "propulsion", "thrust chamber", "aerospace"],
    '''\
PARAMS = {
    "chamber_radius": 32.0,
    "throat_radius": 12.0,
...
# 14. Split Pillow Block Bearing Housing Base
add_ex(
    "pillow_block_bearing_housing",
    "Split journal pillow block bearing housing pedestal base with semicircular bearing seat, twin slotted foot mounting holes, and cap bolt bosses",
    ["pillow block", "bearing housing", "pedestal", "split bearing", "journal bearing", "bearing mount", "machine base"],
    '''\
PARAMS = {
    "base_length": 140.0,
    "base_width": 45.0,
...
# 15. Structural C-Channel Beam Profile
add_ex(
    "structural_c_channel_beam",
    "Standard structural steel C-channel profile section with upper and lower flanges, central web plate, and web mounting slots",
    ["c-channel", "structural beam", "channel iron", "beam profile", "structural steel", "chassis rail", "framing"],
    '''\
PARAMS = {
    "channel_height": 80.0,
    "flange_width": 40.0,
...
# 16. Worm Gear Wheel Blank
add_ex(
    "worm_gear_wheel_blank",
    "Machined bronze worm wheel gear blank with concave throated outer rim, central drive hub, shaft bore, and keyway",
    ["worm gear", "worm wheel", "gear blank", "throated rim", "worm drive", "speed reducer", "bronze gear"],
    '''\
PARAMS = {
    "pitch_radius": 50.0,
    "throat_radius": 46.0,
...
# 17. Vented Louvered Panel
add_ex(
    "vented_louvered_panel",
    "Rectangular equipment ventilation cover panel with angled cooling louver slats and perimeter mounting holes",
    ["louver", "vented panel", "ventilation", "grille", "cooling slats", "sheet metal cover", "cabinet panel"],
    '''\
PARAMS = {
    "panel_length": 120.0,
    "panel_width": 80.0,
...
# 18. Camshaft with Eccentric Cam Lobe
add_ex(
    "camshaft_eccentric_lobe",
    "Automotive internal combustion camshaft segment with circular base journal shaft and asymmetric teardrop cam lobe",
    ["camshaft", "cam lobe", "eccentric cam", "valvetrain", "engine cam", "follower", "timing shaft"],
    '''\
PARAMS = {
    "shaft_dia": 24.0,
    "shaft_length": 80.0,
...
# 19. Exhaust Manifold Cylinder Head Flange
add_ex(
    "exhaust_manifold_flange_3port",
    "Three-port automotive inline engine exhaust manifold interface flange plate with circular exhaust ports and cylinder head mounting stud holes",
    ["exhaust flange", "manifold flange", "engine exhaust", "cylinder head", "exhaust header", "gasket plate", "automotive"],
    '''\
PARAMS = {
    "port_pitch": 75.0,
    "port_dia": 36.0,
...
# 20. Robotics 2-Axis Wrist Clevis Bracket
add_ex(
    "robotics_wrist_clevis",
    "Dual-fork 2-axis robotic arm wrist clevis bracket with base actuator mounting plate, twin fork arms, and cross pivot pin bores",
    ["robotics clevis", "wrist bracket", "clevis fork", "robot arm", "actuator mount", "servo clevis", "end effector"],
    '''\
PARAMS = {
    "base_dia": 50.0,
    "base_thickness": 8.0,
...
# 21. Folding Drone Propeller Rotor Hub
add_ex(
    "folding_propeller_hub",
    "Aerospace folding propeller central rotor hub with two opposed blade hinge clevis forks, center motor drive bore, and blade stop pins",
    ["propeller hub", "folding prop", "rotor hub", "drone propeller", "uav", "aeronautics", "clevis hinge", "quadcopter"],
    '''\
PARAMS = {
    "hub_length": 65.0,
    "hub_width": 20.0,
...
# 22. CNC Leadscrew Flanged Nut Housing
add_ex(
    "leadscrew_nut_flange_housing",
    "CNC 3D printer anti-backlash T8 leadscrew brass flanged nut mounting block with clamping collar and linear carriage mounting pattern",
    ["leadscrew nut", "t8 leadscrew", "nut housing", "cnc", "3d printer", "linear motion", "z-axis", "flanged nut"],
    '''\
PARAMS = {
    "block_length": 34.0,
    "block_width": 30.0,
...
# 23. Pneumatic Cylinder Tie-Rod Front End Cap
add_ex(
    "pneumatic_cylinder_end_cap",
    "ISO square pneumatic cylinder front tie-rod end cap with central piston rod bushing bore, wiper recess, and 4 corner tie-rod holes",
    ["pneumatic cylinder", "end cap", "tie rod", "air cylinder", "actuator cap", "piston rod bushing", "fluid power"],
    '''\
PARAMS = {
    "cap_size": 60.0,
    "cap_thickness": 20.0,
...
# 24. Epicyclic Planetary Gearbox Carrier Plate
add_ex(
    "planetary_gearbox_carrier",
    "Precision epicyclic planetary gearbox carrier plate with central sun shaft clearance bore, 3 equidistant planet gear pin support holes, and perimeter weight reduction pockets",
    ["planetary gearbox", "carrier plate", "planet carrier", "gear train", "epicyclic", "sun gear", "transmission"],
    '''\
PARAMS = {
    "carrier_dia": 85.0,
...
# 25. Axial Piston Pump Swashplate Cradle
add_ex(
    "swashplate_piston_pump_cradle",
    "Variable displacement axial piston hydraulic pump swashplate cradle with precision angled thrust slipper bearing face and side trunnion pivot journals",
    ["swashplate", "axial piston pump", "hydraulic pump", "cradle", "trunnion", "thrust face", "fluid power"],
    '''\
PARAMS = {
    "plate_length": 80.0,
    "plate_width": 65.0,
...
# 26. Metric Slotted Spanner KM Locknut Blank
add_ex(
    "bearing_km_locknut_blank",
    "Standard metric slotted spanner KM locknut blank with central threaded bore clearance, chamfered rim, and 4 perimeter hook spanner slots",
    ["km locknut", "locknut", "bearing nut", "spanner slot", "shaft retention", "bearing retaining", "threaded nut"],
    '''\
PARAMS = {
    "outer_dia": 42.0,
    "nut_thickness": 9.0,
...
# 27. Hexagonal PCB Standoff Spacer
add_ex(
    "hexagonal_threaded_standoff",
    "Machined brass hexagonal PCB standoff spacer with regular hexagonal outer flats and central clearance through-bore",
    ["hex standoff", "spacer", "hexagonal standoff", "pcb hardware", "fastener", "chassis spacer", "threaded post"],
    '''\
PARAMS = {
...
# 28. Centrifugal Pump Volute Casing Section
add_ex(
    "centrifugal_pump_volute_section",
    "Centrifugal fluid pump volute spiral casing section with axial suction inlet eye, expanding scroll gallery, and tangential discharge nozzle flange",
    ["pump volute", "centrifugal pump", "pump casing", "discharge nozzle", "impeller housing", "fluid power", "turbomachinery"],
    '''\
PARAMS = {
...
# 29. GT2 Flanged Timing Belt Pulley
add_ex(
    "timing_pulley_gt2_flanged",
    "Precision GT2 profile timing belt pulley with dual side retaining flange guides, central hub, shaft bore, and setscrew hole",
    ["timing pulley", "gt2", "timing belt", "3d printer pulley", "stepper motor pulley", "synchronous drive", "toothed pulley"],
    '''\
PARAMS = {
    "pitch_radius": 18.0,
    "belt_width": 10.0,
...
# 30. Grooved Thrust Bearing Washer Race
add_ex(
    "thrust_bearing_grooved_race",
    "Precision ball/roller thrust bearing grooved washer race plate with circular ball track guide groove and locating shaft bore",
    ["thrust bearing", "bearing race", "ball track", "thrust washer", "bearing ring", "axial bearing", "rotary table"],
    '''\
PARAMS = {
    "outer_radius": 40.0,
    "inner_radius": 20.0,
...
def validate_all():
    print(f"Total examples to validate: {len(EXAMPLES_DATA)}")
    import tempfile
    with tempfile.TemporaryDirectory() as tmpdir:
        stl_path = str(Path(tmpdir) / "out.stl")
        step_path = str(Path(tmpdir) / "out.step")
        
        for idx, ex in enumerate(EXAMPLES_DATA, 1):
            print(f"[{idx}/{len(EXAMPLES_DATA)}] Testing {ex['id']}...", end=" ", flush=True)
            # Exec code with dummy outputs
            loc = {
                "OUTPUT_STL": stl_path,
            ...
            try:
                exec(ex["code"], loc)
                part = loc.get("part")
                if part is None or part.part is None:
                    raise ValueError("part.part is None")
                if not part.part.is_valid:
                    raise ValueError("part geometry is invalid")
                solids = part.part.solids()
                if len(solids) == 0:
                    raise ValueError("no solids produced")
                print(f"OK ({len(solids)} solid(s), valid={part.part.is_valid})")
            except Exception as e:
                print(f"FAILED: {e}")
                raise

def write_corpus_file():
    output_path = backend_dir / "rag_corpus" / "examples_complex.py"
    print(f"\nWriting to {output_path}...")
    with open(output_path, "w", encoding="utf-8") as f:
        f.write('"""\n')
        f.write('RAG Corpus — 30 High-Difficulty Engineering & Mechanical CAD Examples\n')
        f.write('Covers heatsinks, cylindrical motor housings, gear blanks, pulleys,\n')
        f.write('weld flanges, turbine discs, enclosures, manifolds, camshafts, and actuators.\n')
        f.write('"""\n\n')
        f.write('EXAMPLES = [\n')
        for ex in EXAMPLES_DATA:
            f.write('    {\n')
            f.write(f'        "id": {repr(ex["id"])},\n')
            f.write(f'        "description": {repr(ex["description"])},\n')
            f.write(f'        "tags": {repr(ex["tags"])},\n')
            f.write('        "code": """\\\n')
            f.write(ex["code"])
            f.write('"""\n')
            f.write('    },\n')
        f.write(']\n')
    print("Done writing examples_complex.py!")
